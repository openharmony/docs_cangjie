...
| :-------- | :-------- | :-------- | :-------- |  
| <!--DelRow-->AES/ECB/NoPadding<br/>AES/ECB/PKCS7 | In ECB mode, if NoPadding is selected, the plaintext data must be encrypted in fixed-length blocks. If the input data length is not a multiple of 16, the application must handle padding to meet the block length requirement. | 15+ | No |  
| AES/CBC/NoPadding<br/>AES/CBC/PKCS7<br/>AES/CTR/NoPadding | IV parameter is mandatory. In CBC mode, if NoPadding is selected, the plaintext data must be encrypted in fixed-length blocks. If the input data length is not a multiple of 16, the application must handle padding to meet the block length requirement. | 15+ | Yes |  
