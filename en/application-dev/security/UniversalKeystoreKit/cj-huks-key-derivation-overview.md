...
Derived keys are session key results obtained by services through a three-stage process. Services can decide whether the derived keys are managed by HUKS (i.e., keys do not leave the [TEE](./cj-huks-concepts.md)) or independently managed by the service.  
