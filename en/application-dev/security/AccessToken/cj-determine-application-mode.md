...
1. **Confirm the permission type** of the target permission. This can be verified by searching the corresponding permission list page.
...
If an application needs to declare its APL level as `system_basic` or higher, the `HarmonyAppProvision` configuration file must be modified during the development of the installation package. Specifically, update the `UnsgnedReleasedProfileTemplate.json` file in the SDK directory (`Toolchains / _{Version} _/ lib / UnsgnedReleasedProfileTemplate.json`) and re-sign the application.
...
> **Note:**
>
> Directly modifying the `HarmonyAppProvision` configuration file is only for application debugging and cannot be used for publishing applications to app markets. For commercial applications, apply for release certificates and Profile files in the corresponding app market.
<!--DelEnd-->