# Obtaining Interoperability Module Names on the ArkTS Side

The Cangjie application supports adding ArkTS modules but does not support adding ArkTS pages. The following primarily introduces the addition of ArkTS modules.

ArkTS modules are mainly divided into NAPI (Node.js API) modules and third-party library modules. Cangjie only supports calls to NAPI modules.

Before detailing the specific loading process, we first introduce the mapping specifications between Cangjie import names and ArkTS module names, as follows:
