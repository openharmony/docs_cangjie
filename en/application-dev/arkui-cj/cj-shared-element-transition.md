...
There are multiple implementation methods for the one-shot animation effect. During actual development, appropriate methods should be selected based on specific scenarios.
...
| Using geometryTransition for shared element transition | Leverages system capabilities. Both components before and after transition call the geometryTransition interface with the same ID, while placing the transition logic within the animateTo animation closure. The system will automatically apply the one-shot transition effect between them. | The system adjusts the width, height, and position of the two bound components to identical values and switches their opacity to achieve the one-shot transition effect. Therefore, to ensure smooth animation, it's necessary to guarantee no abrupt changes in width/height animation for nodes bound with geometryTransition. This method is suitable for scenarios where creating new nodes has minimal overhead. |
...
1. Build the page to be expanded and construct interfaces for both normal and expanded states using state variables.

2. Expand the target page, control the disappearance/appearance of sibling components through state variables, and implement transition effects for sibling components through bound appear/disappear transitions.

Take the scenario of clicking a card to display its content details as an example:
...
class EntryView {
    ...
    private var allPostData: Array<PostData> = [
        ...
        PostData(@r(app.media.startIcon),"Carl","Everything grows",[@r(app.media.startIcon),@r(app.media.startIcon),@r(app.media.startIcon)])
        ]
        ...
        Column(){
            ...
            if(this.isExpand){
                Column(){
                    ...
                    // Add appear/disappear transition effect for comment section text
        ...
        .onClick({
            evt =>
                ...
                getUIContext().animateTo(AnimateParam(duration:350,curve:Curve.Friction),{
                    =>
                    // Apply width/height animation for expanded posts, and animations for avatar size and image size
                    this.isExpand = !this.isExpand
                    if(this.isExpand){
                        this.itemHeight = 780
                        this.avatarSize = 75
...
[geometryTransition](../reference/arkui-cj/cj-animation-geometrytransition.md) is used for implicit shared element transitions within components, providing smooth context inheritance during view state transitions.

To use geometryTransition, bind two components that need the one-shot effect with the same ID through the geometryTransition interface. When one component disappears while another appears, the system will apply the one-shot effect between them.
...
For a one-shot effect between two elements in the same page, here's a simple example of using the geometryTransition interface:
    ...
    func build() {
        Stack(alignContent: Alignment.Center) {
            if (this.isShow) {
                ...
                // It won't affect child components' borderRadius inside the container
                .borderRadius(20)
                .clip(true)
                .position(x:40,y:40)
                .geometryTransition("picture")
...
In more scenarios, one-shot effects need to be applied between elements from different pages. This can be achieved by combining geometryTransition with modal transition interfaces. Take the demo of clicking an avatar to display a personal information page as an example:
...
The effect is that after clicking the avatar on the homepage, a modal page pops up to display personal information, with a seamless transition animation for the avatar between the two pages:
