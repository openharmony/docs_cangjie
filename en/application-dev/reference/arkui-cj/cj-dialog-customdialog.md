...
|shadow|?[ShadowOptions](./cj-common-types.md#class-shadowoptions)|No|None| **Named parameter.** Sets the shadow of the dialog backplane.<br>Initial value:<br>Before API version 26, the initial value is ShadowOptions(radius: 0.0);<br>From API version 26 onward, the initial value is ShadowOptions(radius: -1.0).|
