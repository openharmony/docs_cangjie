...
All the above permissions require user authorization via a pop-up dialog. For specific request and verification methods, refer to [Requesting User Authorization](../../../../en/application-dev/security/AccessToken/cj-request-user-authorization.md#向用户申请授权).

> **Note:**
>
> Only when the application needs to clone, back up, or synchronize image and video files in the user's public directory, the ohos.permission.READ_IMAGEVIDEO and ohos.permission.WRITE_IMAGEVIDEO permissions can be requested to read and write audio files. For the request method, refer to [Requesting Restricted Permissions](../../../../en/application-dev/security/AccessToken/cj-declare-permissions-in-acl.md#申请使用受限权限).
