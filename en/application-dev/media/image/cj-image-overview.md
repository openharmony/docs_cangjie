...
3. **[Image Decoding](./cj-image-decoding.md)**: Decode the image into a PixelMap via ImageSource.
4. **[Image Processing](./cj-image-transformation.md)**: Process the PixelMap to alter image properties, achieving effects like rotation, scaling, and cropping. Then display the image using the [Image component](../../arkui-cj/cj-graphics-display.md).
5. **[Image Encoding](./cj-image-encoding.md)**: Use the ImagePacker class to compress and encode the PixelMap or ImageSource, generating a new image.
...
The image framework provides image encoding and decoding capabilities, supporting components like the Image component and gallery applications. The decoded results can be passed to the [Image component](../../arkui-cj/cj-graphics-display.md) for display.