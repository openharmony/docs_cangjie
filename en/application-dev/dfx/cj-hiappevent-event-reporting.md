...
For detailed usage instructions of the API interfaces (parameter usage constraints, specific value ranges, etc.), please refer to the [Application Event Tracking API Documentation](../reference/PerformanceAnalysisKit/cj-apis-hiappevent.md).
...
Taking the example of tracking button click events and reporting them via processors, the development steps are as follows:

1. Edit the "entry > src > main > cangjie > index.cj" file in the project, add a button, and include a data processor in its onClick function. `analytics_demo` is a preset data processor library on the device. The complete sample code is as follows:
...
2. Edit the "entry > src > main > cangjie > index.cj" file in the project, add a button, and include functions to set and retrieve the user ID in its onClick function. The complete sample code is as follows:
    ...
    ```cangjie
        Button("userIdTest").onClick ({
                evt =>
                // Set the user ID in the button click function
...
3. Edit the "entry > src > main > cangjie > index.cj" file in the project, add a button, and include functions to set and retrieve user properties in its onClick function. The complete sample code is as follows:
    ...
    ```cangjie
        Button("userPropertyTest").onClick ({
            evt =>
            // Set the user property value in the button click function
            HiAppEvent.setUserProperty('testUserPropertyName', '123456')

            // Retrieve the set user property value in the button click function
            let userProperty = HiAppEvent.getUserProperty('testUserPropertyName')
            Hilog.info(0x0000, 'testTag', 'userProperty: ${userProperty}')
        })
    ```

4. Edit the "entry > src > main > cangjie > index.cj" file in the project, add a button, and include event tracking in its onClick function to record button click events. The complete sample code is as follows:
...
5. Edit the "entry > src > main > cangjie > index.cj" file in the project, add a button, and include the function to remove a data processor in its onClick function (the data processor was added in Step 2). The complete sample code is as follows:
...
6. Click the Run button in the IDE to launch the application project. Then, sequentially click the buttons "addProcessorTest", "userIdTest", "userPropertyTest", "writeTest", and "removeProcessorTest" in the application interface to successfully report an event via the data processor.
