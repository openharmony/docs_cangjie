...
ForEach接口基于数组类型数据来进行循环渲染，需要与容器组件配合使用，且接口返回的组件应当是允许包含在ForEach父容器组件中的子组件。例如，ListItem组件要求ForEach的父容器组件必须为[List组件](../../../../zh-cn/application-dev/reference/arkui-cj/cj-scroll-swipe-list.md)。
...
- ForEach在下列容器组件 [List](../../../../zh-cn/application-dev/reference/arkui-cj/cj-scroll-swipe-list.md)、[Grid](../../../../zh-cn/application-dev/reference/arkui-cj/cj-scroll-swipe-grid.md)、[Swiper](../../../../zh-cn/application-dev/reference/arkui-cj/cj-scroll-swipe-swiper.md)内使用的时候，不要与LazyForEach 混用。 以List为例，同时包含ForEach、LazyForEach的情形是不推荐的。
