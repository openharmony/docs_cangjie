...
仓颉- ArkTS 互操作从使用场景上分为仓颉应用中使用 ArkTS 及 ArkTS 应用中使用仓颉两种情况。从调用方式上区分，可以分为 ArkTS 调用仓颉，以及仓颉调用ArkTS。 ArkTS 调用仓颉可以分为用互操作宏进行开发以及用互操作库进行开发。仓颉调用 ArkTS分为两种情况，调用 ArkTS 系统库和调用 ArkTS 三方库，调用系统库通过动态导入的方式进行开发，调用三方库通过HLE工具的方式进行开发。区分如下表：
