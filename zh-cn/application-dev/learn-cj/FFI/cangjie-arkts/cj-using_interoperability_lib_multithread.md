...
下面通过一个示例来展示具体做法。该示例为一个互操作函数，其功能是对两个数字相加，并调用回调函数返回相加结果。
...
在 ArkTS 中存在 Promise，这是对回调机制的一种封装，配合 async、await 的语法让回调机制变成同步调用的形式。对于上一个示例，可以使用 Promise 的形式来定义接口和访问：
