...
3. 三级目录el1/~el5/：代表不同文件加密类型。
   ...
   >
   > 开发者可通过监听[COMMON_EVENT_USER_UNLOCKED](../../../zh-cn/application-dev/reference/BasicServicesKit/cj-apis-common_event_manager.md#static-const-common_event_user_unlocked)事件感知当前用户首次认证完成。
...
4. 四级、五级目录：
   ...
   | ---------------- | ------------------- | ------------------ | -------------------- |
   | bundle           | bundleCodeDir       | 安装文件路径       | 应用安装后的App的HAP资源包所在目录；随应用卸载而清理。<br>不能拼接路径访问资源文件，请使用[资源管理接口](../../../zh-cn/application-dev/reference/LocalizationKit/cj-apis-resource_manager.md)访问资源。<br>可以用于存储应用的代码资源数据，主要包括应用安装的HAP资源包、可重复使用的库文件以及插件资源等。此路径下存储的代码资源数据可以被用于动态加载。                                                                                           |
